...
public class estTree {
		...
		for (estNode node : root.children) {
			...
			if (s > minsup) {
				...
				// if store into file
                                ...
                                int Threshold = 5;
				if(patterns == null) {
                                       //System.out.println("Suspcious Activity Found"+suspCount +  " MinSup  "+s);
					writeItemset(patt2, s);
                                        sum = suspCount+sum;
                                        suspCount += 1;
                                        if (sum>Threshold){
                                          System.out.println("Alter :Suspcious Activity Found");
	...
	void writeItemset(List<Integer> itemset, double support) throws IOException {
		...
		buffer.append(support);
                System.out.println("Suspicious Patterns Found"+buffer.toString());
